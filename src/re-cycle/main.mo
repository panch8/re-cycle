...
actor {
  ...
  let adminWhiteList : [Text] = [
    "peqgi-d5vlz-yq43p-iol25-vlp5p-bcvja-3taz3-tm45v-jzb4e-u4wlz-2qe",//fran.codino on mainnet
  ...
  public shared ({caller}) func getCanistersByController(controllerId: Text) : async ?Types.CanisterResponse {
    let admins = Buffer.fromArray<Text>(adminWhiteList);
    // as is released in Mainnet, i have restricted this call to a whitellist of principals.
    assert (Buffer.contains<Text>(admins, Principal.toText(caller),Text.equal));
